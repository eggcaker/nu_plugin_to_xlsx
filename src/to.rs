    ...
    match &value {
        Value::Record { val, .. } => {
            for (column, cn) in val.cols.iter().enumerate() {
                let _ = worksheet.write_string(0, column as ColNum, cn);
            }

            for (column, v) in val.vals.iter().enumerate() {
                write_value(worksheet, 1, column as ColNum, &v).unwrap();
            }
        }
            ...
            for (row, val) in vals.iter().enumerate() {
                match val {
                    Value::Record { val, .. } => {
                        ...
                        for (column, v) in val.vals.iter().enumerate() {
                            write_value(worksheet, (row + 1) as RowNum, column as ColNum, &v)
                                .unwrap();
                        }
                    }
...
fn write_value(
    ...
    let _ = match value {
        ...
        Value::Int { val, .. } => worksheet.write_number_with_format(
            row,
            col,
            *val as f64,
        ...
        Value::Date { val, .. } => worksheet.write_string(row, col, &val.format("%s").to_string()),

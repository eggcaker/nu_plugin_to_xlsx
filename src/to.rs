...
pub(crate) fn write_to_xlsx(_document: &Value, path: &str) -> Result<(), LabeledError> {
    let mut workbook = Workbook::new();
    let worksheet = workbook.add_worksheet();

    let val = Value::String { val: String::from("hello"), internal_span: Span::unknown() };
    let world= Value::String { val: String::from("world"), internal_span: Span::unknown() };
...
fn write_value(worksheet: &mut Worksheet, row: RowNum, col: ColNum, value: &Value) -> Result<(), LabeledError> {
    let _ =  match value {
        ...
        Value::Bool { val, .. } => worksheet.write_string(row, col, if *val { "true" } else { "false" } ),
